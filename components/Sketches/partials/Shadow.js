...
const Shadow = dynamic(() => import("react-p5").then((mod) => mod.default), {
  ssr: false,
});
const size = 15;
const w = 10;
...
export default ({ data, bg }) => {
  const setup = (p5, canvasParentRef) => {
    ...
    p5.createCanvas(800, _height).parent(canvasParentRef);
    ...
    p5.drawingContext.shadowBlur = 4;
  ...
  const keyPressed = (p5, { key }) => {
    if (key === "p" || key === "P") {
      ...
      pg.drawingContext.shadowOffsetX = -5 * s;
      pg.drawingContext.shadowOffsetY = 4 * s;
      pg.drawingContext.shadowBlur = 4 * s;
